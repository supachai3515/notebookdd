...
class Products_model extends CI_Model {
	...
	{
		$sql ="SELECT * FROM branch WHERE is_active = 1 ORDER BY name"; 
		$result = $this->db->query($sql);
		return  $result->result_array();
	}
	...
	public function save_product()
	{
		...
		$data_product = array(
			...
			'is_add_outofstock' =>  $this->input->post('is_add_outofstock'),
    ...
    public function get_products_search()
	{
		...
		if($data_product['is_reservations'] !='' ){
			$sql = $sql."AND (p.is_reservations = '".$data_product['is_reservations']."')";
		}

		if($data_product['is_add_outofstock'] !='' ){
			$sql = $sql."AND (p.is_add_outofstock = '".$data_product['is_add_outofstock']."')";
		}
