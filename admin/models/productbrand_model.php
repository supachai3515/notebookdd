...
class Productbrand_model extends CI_Model {
	...
	{
		$sql ="SELECT * FROM product_type WHERE id = '".$productbrand_id."'"; 
    ...
    public function get_productbrand_search()
	{
		...
		$sql ="SELECT * FROM product_type WHERE name LIKE '%".$data_productbrand['search']."%' OR  description LIKE '%".$data_productbrand['search']."%' ";
		$re = $this->db->query($sql);
	...
	public function update_productbrand($productbrand_id)
	{
		...
		$this->db->update("product_type", $data_productbrand, $where);
