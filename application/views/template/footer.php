<div id="footer">
    <div class="footer">
        <div class="container">
            <div class="row">
                <div class="col-md-4 col-sm-4 col-xs-12">
                    <div class="logo logo-footer">
                        <a href="<?php echo base_url() ?>" class="sub-title"><span class="lnr lnr-apartment"></span> notebookdd</a>
                    </div>
                    <div class="contact-phone">
                        <label class="sub-title"><i class="fa fa-map-marker" aria-hidden="true"></i> ADDRESS:</label>
                        <span class="phone-number">135/99 ถ.สุขุมวิท ต.ศรีราชา อ.ศรีราชา จ.ชลบุรี 20110 ชั้น 2 ตึกคอมศรีราชา</span>
                    </div>
                    <div class="contact-phone">
                        <label class="sub-title"><i class="fa fa-phone" aria-hidden="true"></i> hot-line:</label>
                        <span class="phone-number">061 478 8949</span>
                        <span class="phone-number">089 507 2023</span>
                        <span class="phone-number">090 983 9636</span>
                    </div>
                    <div class="contact-phone">
                        <label class="sub-title"><i class="fa fa-comment"></i> line id:</label>
                        <span class="phone-number"><a href="http://line.me/ti/p/%40zlg9137d" target="_blank">@notebookdd</a></span>
                    </div>
                    <div class="contact-phone">
                        <label class="sub-title"><i class="fa fa-facebook-official"></i> facebook:</label>
                        <span class="phone-number"><a href="https://www.facebook.com/notebooksimple/" target="_blank">notebooksimple</a></span>
                    </div>
                </div>
                <div class="col-md-4 col-sm-4 col-xs-12">
                    <div class="footer-nav">
                        <h2 class="sub-title">Account</h2>
                        <ul class="list-unstyled">
                            <li><a href="<?php echo base_url('products')?>"><i class="fa fa-laptop"></i> ร้านค้า</a></li>
                            <li><a href="<?php echo base_url('howtobuy')?>"><i class="fa fa-shopping-bag"></i> วิธีการสั่งซื้อ</a></li>
                            <li><a href="<?php echo base_url('payment')?>"><i class="fa fa-money"></i> แจ้งชำระเงิน</a></li>
                            <li><a href="<?php echo base_url('tracking')?>"><i class="fa fa-truck"></i> tracking</a></li>
                            <li><a href="<?php echo base_url('warranty')?>">เงื่อนไขการรับประกัน</a></li>
                            <li><a href="<?php echo base_url('download')?>">Dowload เอกสาร</a></li>
                        </ul>
                    </div>
                </div>
                <div class="col-md-4 col-sm-4 col-xs-12">
                    <div class="newsletter">
                        <h2 class="sub-title">เวลาทำการ notebookdd.com</h2>
                        <p><i class="fa fa-clock-o"></i> ทุกวัน 10.30 - 20.00 น.</p>
                        <h2 class="sub-title">ช่องทางการชำระเงิน</h2>
                        <p><a href="<?php echo base_url('payment')?>"> <img src="<?php echo base_url() ?>/theme/images/bankimg.jpg" class="img-responsive img-rounded" alt="Image" width="100%"></a></p>
                    </div>
                    
                </div>
            </div>
        </div>
    </div>
</div>
<!-- End Footer -->
</div>